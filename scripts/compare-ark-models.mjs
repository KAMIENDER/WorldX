...
const BASE_URL = process.env.ARK_BASE_URL || "https://ark.cn-beijing.volces.com/api/coding/v3";
const API_KEY = process.env.ARK_API_KEY || "";
const TIMEOUT_MS = parseInt(process.env.ARK_REQUEST_TIMEOUT_MS || "30000", 10);
...
async function main() {
  ...
  for (const model of MODELS) {
    console.log(`=== ${model} ===`);
    const plainResult = await callArk({
      model,
    ...
    const jsonResult = await callArk({
      model,
...
async function callArk({ model, messages, jsonMode, label }) {
  const body = {
    model,
    messages,
    temperature: 0,
